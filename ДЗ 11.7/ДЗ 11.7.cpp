...
/*
Утечки памяти в коде! Не освобождается память в методе -> void sort_func 
В предложенном варианте если один массив пуст а второй нет 
произойдет выход без освобождения памяти! 
И как следствие даже добавит в файл то, что находится в непустом массиве, что приведет к потере данных!
...
После вызова метода: merge_to_file(part_1, part_2, size_1, size_2); дописать: Для освобождения динамической памяти.

  delete[] part_1;
  delete[] part_2;
2.Метод : void read_arr(const string& filename, int*& arr, const int& n)

Лишний и вообще не используется!
*/